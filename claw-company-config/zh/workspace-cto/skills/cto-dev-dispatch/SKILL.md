...
- **強制觸發條件**：涉及安全、金流、多系統整合、或用戶資料處理 → 一律 spawn QA 規劃測試策略
...
每個 spawn 任務的 task 欄位**必須**包含以下結構：
...
| 任務類型 | 注入的 rules |
|---------|-------------|
| 新功能開發（Dev） | `rules/tdd-iron-law.md` + `rules/verification.md` + `rules/sdd-iron-law.md` |
| Bug 修復（Dev） | `rules/debugging-iron-law.md` + `rules/tdd-iron-law.md` + `rules/verification.md` |
| 重構（Dev） | `rules/tdd-iron-law.md` + `rules/verification.md` |
| 規格類任務（PM/架構師） | `rules/sdd-iron-law.md` + `rules/verification.md` |
| 測試類任務（QA） | `rules/tdd-iron-law.md` + `rules/verification.md` |
| 審查類任務（Scout/Knox） | 依工程師定義檔案的 `rules` 欄位 |

鐵律路徑格式：`{{INSTALL_DIR}}/workspace-cto/rules/{rule-name}.md`
