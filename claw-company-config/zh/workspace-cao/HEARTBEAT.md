...
### 緊急標記掃描
- 掃描各 Agent 的 `output/` 目錄，優先檢查 `URGENT-` 前綴檔案（Cron 環境產生的 P0 事件標記）
- 發現 URGENT 檔案 → 立即讀取內容、通知 CEO + 董事長、建立 P0 稽核議題
- 處理完畢後將 URGENT 檔案移至 `output/archive/`
...
掃描 CTO 的 `status.md`，檢查進行中任務的 spawn 數量：

- **有流程等級標記**（精簡/完整）→ 對比上限（精簡 ≤ 4 / 完整 ≤ 12）
- **無流程等級標記** → 視為精簡（上限 4）
- **無法解析記錄格式** → 記錄為 P2 稽核議題，通知 CTO 修正 status.md 格式
- **超過上限** → exec dispatch 通知 CEO 調查（write 寫檔 → dispatch.sh），記錄為 P2 稽核議題
- **超過 OpenClaw 硬限制 12** → 不可能（runtime 已擋），但如果偵測到接近上限的連續 spawn 模式，記錄觀察
